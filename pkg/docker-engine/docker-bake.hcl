...
# Sets the docker engine repo. Will be used to clone the repo at
# DOCKER_ENGINE_VERSION ref to include the README.md and LICENSE for the
# static packages and also create version string.
variable "DOCKER_ENGINE_REPO" {
  default = "https://github.com/docker/docker.git"
}

# Sets the docker engine helpers version to build from source.
...
target "_common" {
  ...
  cache-from = [
    BUILD_CACHE_REGISTRY_SLUG != "" ? "type=registry,ref=${BUILD_CACHE_REGISTRY_SLUG}:docker-engine-${PKG_TYPE}-${PKG_RELEASE}" : "",
  ]
  cache-to = [
    BUILD_CACHE_REGISTRY_SLUG != "" && BUILD_CACHE_REGISTRY_PUSH != "" ? "type=registry,ref=${BUILD_CACHE_REGISTRY_SLUG}:docker-engine-${PKG_TYPE}-${PKG_RELEASE},mode=max" : "",
  ]
}
...
target "meta-helper" {
  tags = ["dockereng/packaging:docker-engine-local"]
}
