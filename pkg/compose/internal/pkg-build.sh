...
if [ "$PKG_TYPE" = "static" ]; then
  ...
  if [ "$(xx-info os)" = "windows" ]; then
    (
      ...
      cd "$workdir"
      zip -r "$pkgoutput/${PKG_NAME}_${COMPOSE_VERSION#v}.zip" ${PKG_NAME}
    )
  else
    (
      set -x
      cp /usr/bin/compose "$workdir/${PKG_NAME}/docker-compose"
      tar -czf "$pkgoutput/${PKG_NAME}_${COMPOSE_VERSION#v}.tgz" -C "$workdir" ${PKG_NAME}
    )
  fi
  ...
  (
    set -x
    ARCH="${arch}" VERSION="${COMPOSE_VERSION}" RELEASE="$PKG_REVISION" VENDOR="${PKG_VENDOR}" PACKAGER="${PKG_PACKAGER}" nfpm package --config ./nfpm.yml --packager "$PKG_TYPE" --target "$pkgoutput"
  )
else
  rm -rf "${pkgoutput}"
fi
