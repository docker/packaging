...
# Sets the buildkit repo. Will be used to clone the repo at
# BUILDKIT_REF to include the README.md and LICENSE for the
# static packages and also create version string.
variable "BUILDKIT_REPO" {
  default = "https://github.com/moby/buildkit.git"
}

# Sets the buildkit ref.
variable "BUILDKIT_REF" {
  default = "master"
}

# In case we want to set runc to a specific version instead of using
# the one used by buildkit
variable "RUNC_REPO" {
...
target "_common" {
  ...
  cache-from = [
    BUILD_CACHE_REGISTRY_SLUG != "" ? "type=registry,ref=${BUILD_CACHE_REGISTRY_SLUG}:buildkit-${PKG_TYPE}-${PKG_RELEASE}" : "",
  ]
  cache-to = [
    BUILD_CACHE_REGISTRY_SLUG != "" && BUILD_CACHE_REGISTRY_PUSH != "" ? "type=registry,ref=${BUILD_CACHE_REGISTRY_SLUG}:buildkit-${PKG_TYPE}-${PKG_RELEASE},mode=max" : "",
  ]
}
...
target "meta-helper" {
  tags = ["dockereng/packaging:buildkit-local"]
}
