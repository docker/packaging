...
# Sets the buildx repo. Will be used to clone the repo at
# BUILDX_VERSION ref to include the README.md and LICENSE for the
# static packages and also create version string.
variable "BUILDX_REPO" {
  default = "https://github.com/docker/buildx.git"
}

# Sets the buildx version to build from source.
...
target "_common" {
  ...
  cache-from = [
    BUILD_CACHE_REGISTRY_SLUG != "" ? "type=registry,ref=${BUILD_CACHE_REGISTRY_SLUG}:buildx-${PKG_TYPE}-${PKG_RELEASE}" : "",
  ]
  cache-to = [
    BUILD_CACHE_REGISTRY_SLUG != "" && BUILD_CACHE_REGISTRY_PUSH != "" ? "type=registry,ref=${BUILD_CACHE_REGISTRY_SLUG}:buildx-${PKG_TYPE}-${PKG_RELEASE},mode=max" : "",
  ]
}
