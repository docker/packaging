...
RUN --mount=from=bin,target=/build <<EOT
  ...
  set -x
  actual_version=$(docker --version | awk '{print $3}' | cut -d',' -f1)
  echo "Detected docker CLI version: $actual_version"
  if [ -n "$PKG_VERSION" ]; then
    expected_version=$(echo "$PKG_VERSION" | sed 's/^v//')
    if [ "$actual_version" != "$expected_version" ]; then
      echo "ERROR: Docker CLI version mismatch! Expected: $expected_version, Got: $actual_version"
      exit 1
    fi
    echo "SUCCESS: Docker CLI version verification passed"
  fi
EOT
